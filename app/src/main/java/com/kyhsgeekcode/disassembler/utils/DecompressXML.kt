...
suspend fun decompressXML2(xml: ByteArray): AnnotatedString =
    withContext(Dispatchers.Default) {
        // Compressed XML file/bytes starts with 24x bytes of data,
        // 9 32 bit words in little endian order (LSB first):
        //   0th word is 03 00 08 00
            ...
            while (off < xml.size) {
                ...
                if (tag0 == startTag) { // XML START TAG
                    ...
                    for (ii in 0 until numbAttrs) {
                        ...
                        append(
                            " $attrName",
                            PrettifyHighlighter.getColor("kwd")
                        )
                        appendSpanned(
                            "=\"",
                            PrettifyHighlighter.getColor("pun")
                        )
                        appendSpanned(
                            "$attrValue",
                            PrettifyHighlighter.getColor("lit")
                        )
                        appendSpanned(
                            "\"",
                            PrettifyHighlighter.getColor("pun")
                    ...
                    prtIndent(indent, "")
                    appendSpanned("<", PrettifyHighlighter.getColor("pun"))
                    appendSpanned(
                        "$name",
                        PrettifyHighlighter.getColor("typ")
                    )
                    appendSpanned(">", PrettifyHighlighter.getColor("pun"))
                    append(System.lineSeparator())
                    indent++
                    ...
                    appendSpanned(
                        "(line $startTagLineNo-$lineNo)",
                        PrettifyHighlighter.getColor("com")
                    )
                    append(System.lineSeparator())
                    //tr.parent();  // Step back up the NobTree
                } else if (tag0 == endDocTag) {  // END OF XML DOC TAG
...
fun AnnotatedString.Builder.prtIndent(indent: Int, str: String) {
    append(spaces.substring(0, (indent * 2).coerceAtMost(spaces.length)) + str)
}

fun AnnotatedString.Builder.append(str: String, color: Color) {
    withStyle(SpanStyle(color)) {
        append(str)
    }
}

fun AnnotatedString.Builder.append(str: String, color: Int) {
    append(str, Color(color))
}

fun AnnotatedString.Builder.appendSpanned(text: String, color: Int) {
    append(text, color)
}
