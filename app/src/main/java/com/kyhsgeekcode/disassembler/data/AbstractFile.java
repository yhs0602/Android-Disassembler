...
public abstract class AbstractFile implements Closeable {
    ...
    public static AbstractFile createInstance(String tag) throws IOException {
        ...
        //그러면 중복코드도 사라짐
        //행복회로
        byte[] content = Util.getBytes(new FileInputStream(file));
        if (file.getPath().endsWith("assets/bin/Data/Managed/Assembly-CSharp.dll")) {
            ...
            try {
                ...
                if (assembly != null) {
                    ...
                    return new ILAssmeblyFile(facileReflector);
                } else {
            ...
            try {
                return new ELFFile(file, content);
            } catch (Exception e) {
...
//	}
    ...
    public long codeBase = 0;
    public long codeLimit = 0;
    List<Symbol> symbols;
    List<PLT> importSymbols;
    public byte[] fileContents;
    public long entryPoint = 0;
    public long codeVirtualAddress = 0;
